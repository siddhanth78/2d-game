...
import heapq
import random
from grid import W, H, tile_id, ENTITY_ENEMY, ENTITY_SEEKER
...
class Enemy:
    ...
    def update(self, flow_field, enemies, grid):
        ...
        if (dx, dy) != (0, 0):
            ...
            if grid.get_entity(nx, ny) == ENTITY_ENEMY:
                if dx != 0:
                ...
                for ax, ay in alts:
                    if (0 <= ax < W and 0 <= ay < H and
                        ...
                        grid.get_entity(ax, ay) != ENTITY_ENEMY):
                        nx, ny = ax, ay
                        break
                else:
                    nx, ny = self.x, self.y
            ...
            if (nx, ny) != (self.x, self.y):
                ...
                self.x, self.y = nx, ny
                grid.set_entity(self.x, self.y, ENTITY_ENEMY)
...
class FlowField:
    ...
    def draw(self, screen, cell_size):
        ...
        for (x, y), (dx, dy) in self.field.items():
            ...
            surf = font.render(arrow, True, (255, 100, 100))
